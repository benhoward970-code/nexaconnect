...
-- ═══════════════════════════════════════════════════════════════
-- Phase 3: Lead Connection System + Notifications
-- ═══════════════════════════════════════════════════════════════

-- Leads table — participants submit support requests, providers pay to unlock
...
create policy "Providers can read own leads" on leads for select using (
  exists (select 1 from providers where providers.id = leads.provider_id and providers.user_id = auth.uid())
);
create policy "Providers can update own leads" on leads for update using (
  exists (select 1 from providers where providers.id = leads.provider_id and providers.user_id = auth.uid())
);
create policy "Participants can insert own leads" on leads for insert with check (
  exists (select 1 from participants where participants.id = leads.participant_id and participants.user_id = auth.uid())
);
create policy "Participants can read own leads" on leads for select using (
  exists (select 1 from participants where participants.id = leads.participant_id and participants.user_id = auth.uid())
);

-- Notifications table — persistent, replaces ephemeral in-memory notifications
